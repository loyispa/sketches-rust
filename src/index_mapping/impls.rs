...
impl CubicallyInterpolatedMapping {
    ...
    pub fn with_relative_accuracy(relative_accuracy: f64) -> CubicallyInterpolatedMapping {
        let gamma = CubicallyInterpolatedMapping::calculate_gamma(relative_accuracy, CubicallyInterpolatedMapping::CORRECTING_FACTOR);
        let index_offset: f64 = 0.0;
        let multiplier = std::f64::consts::LN_2 / gamma.ln();
        let relative_accuracy = CubicallyInterpolatedMapping::calculate_relative_accuracy(gamma, CubicallyInterpolatedMapping::CORRECTING_FACTOR);
        CubicallyInterpolatedMapping { relative_accuracy, gamma, index_offset, multiplier }
    }
    ...
    pub fn decode(input: &mut impl Input) -> Result<CubicallyInterpolatedMapping, Error> {
        ...
        Ok(CubicallyInterpolatedMapping::with_gamma_offset(gamma, index_offset))
    }
    ...
    fn log(&self, value: f64) -> f64 {
        ...
        return ((CubicallyInterpolatedMapping::A * s + CubicallyInterpolatedMapping::B) * s + CubicallyInterpolatedMapping::C) * s + e;
    }

    fn log_inverse(&self, index: f64) -> f64 {
        let exponent = index.floor() as i64;
        // Derived from Cardano's formula
        let d0 = CubicallyInterpolatedMapping::B * CubicallyInterpolatedMapping::B - 3.0 * CubicallyInterpolatedMapping::A * CubicallyInterpolatedMapping::C;
        let d1 = 2.0 * CubicallyInterpolatedMapping::B * CubicallyInterpolatedMapping::B * CubicallyInterpolatedMapping::B - 9.0 * CubicallyInterpolatedMapping::A * CubicallyInterpolatedMapping::B * CubicallyInterpolatedMapping::C - 27.0 * CubicallyInterpolatedMapping::A * CubicallyInterpolatedMapping::A * (index - exponent as f64);
        let p = ((d1 - (d1 * d1 - 4.0 * d0 * d0 * d0).sqrt()) / 2.0).cbrt();
        let significand_plus_one = -(CubicallyInterpolatedMapping::B + p + d0 / p) / (3.0 * CubicallyInterpolatedMapping::A) + 1.0;
...
impl IndexMapping for CubicallyInterpolatedMapping {
    ...
    fn min_indexable_value(&self) -> f64 {
        f64::max(f64::powf(2.0, (i32::MIN as f64 - self.index_offset) / self.multiplier + 1.0),
                 f64::MIN_POSITIVE * (1.0 + self.relative_accuracy) / (1.0 - self.relative_accuracy))
    }

    fn max_indexable_value(&self) -> f64 {
        f64::max(f64::powf(2.0, (i32::MAX as f64 - self.index_offset) / self.multiplier - 1.0),
...
mod tests {
    ...
    #[test]
    fn test_accuracy() {
        let accuracy = [5.04943011787191E-7, 5.04943011787191E-7, 5.04943011787191E-7, 5.04943011787191E-7, 0.009998870028530763, 0.009998870028530763, 0.009998870028530763, 0.009998870028530763, 0.20192337476263508, 0.20192337476263508, 0.20192337476263508, 0.20192337476263508];
        let mut index = 0;
        for gamma in TEST_GAMMAS {
            for index_offset in TEST_INDEX_OFFSETS {
                let index_mapping = CubicallyInterpolatedMapping::with_gamma_offset(gamma, index_offset);
