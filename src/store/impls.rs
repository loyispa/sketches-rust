...
impl CollapsingLowestDenseStore {
    ...
    fn extend_range(&mut self, new_min_index: i32, new_max_index: i32) {
        ...
        } else if new_min_index >= self.offset
            && new_max_index < self.offset + self.get_length() {
            self.min_index = new_min_index;
            self.max_index = new_max_index;
        } else {
        ...
        if new_max_index - new_min_index + 1 > self.get_length() {
            ...
            if new_min_index >= self.max_index {
                ...
                if shift < 0 {
                    ...
                    let collapsed_count = self.get_total_count_with_range(self.min_index, new_min_index - 1);
    ...
    fn get_new_length(&self, new_min_index: i32, new_max_index: i32) -> i32 {
        ...
        i32::min(self.max_num_bins,
                 ((desired_length + self.array_length_overhead - 1) / self.array_length_growth_increment + 1) * self.array_length_growth_increment)
    }
    ...
    fn shift_counts(&mut self, shift: i32) {
        ...
        self.array_copy(min_array_index, min_array_index + shift, max_array_index - min_array_index + 1);
    ...
    fn array_copy(&mut self, src_pos: i32, dest_pos: i32, length: i32) {
        if src_pos < dest_pos {
            ...
            while offset >= 0 {
                self.counts[(dest_pos + offset) as usize] = self.counts[(src_pos + offset) as usize];
                offset -= 1;
            }
...
impl Store for CollapsingLowestDenseStore {
    fn add(&mut self, index: i32, count: f64) {
        ...
        match array_index {
            Ok(index) => { self.counts[index] += count; }
            _ => {}
        }
    }
    ...
    fn add_bin(&mut self, bin: (i32, f64)) {
        ...
        match array_index {
            Ok(index) => { self.counts[index] += bin.1; }
            _ => {}
        }
    }
